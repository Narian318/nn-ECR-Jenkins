pipeline {
    ...
    sh 'aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin 167613117387.dkr.ecr.us-east-1.amazonaws.com'
     sh 'docker push 167613117387.dkr.ecr.us-east-1.amazonaws.com/nn-ecr-jenkins:$(BUILD_NUMBER)'
}
            ...
            stage('Docker Run') {
              steps{
                   script {
                sh 'docker run -d -p 8096:5000 --rm --name mypythonContainer 167613117387.dkr.ecr.us-east-1.amazonaws.com/nn-ecr-jenkins:$(BUILD_NUMBER)'     
      }
    }
        }
    }
